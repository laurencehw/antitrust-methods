...
Use this table alongside external references such as Ashenfelter and Hosken (2010) and agency guidelines.
...
- Run parallel-trend visuals, dynamic leads/lags, and placebo periods (`did::att_gt`, `fixest::sunab`). Reference Cunningham (2021) for intuition and cite Ashenfelter and Hosken (2010) for merger retrospectives. For staggered treatment timing, see Callaway and Sant'Anna (2021).
- For South African matters with staggered regulatory actions, consider matrix-completion methods from recent econometrics literature (Athey and Imbens, 2017).

### Matching and weighting
- Propensity score, entropy balancing, and synthetic controls should include balance plots (standardized differences, variance ratios) (Abadie, Diamond, and Hainmueller, 2010).
- Store reusable plotting code in `R/helpers.R` (`plot_balance()`). For general guidance on matching methods, see Angrist and Pischke (2009).
