...
```
**Key insight:** X/(X + M) is the *breakeven* critical loss --- the share loss at which a SSNIP of size X stops paying, given margin M. A profit-maximizing variant asks instead whether the hypothetical monopolist would choose to raise price at all; it yields lower thresholds, so say which version you are using. High margins push the breakeven threshold down; low margins push it up.
{% endhint %}

A low critical loss does not, by itself, justify a narrow market. High pre-merger margins are themselves evidence of inelastic demand: the same margins that lower the critical loss also predict a low *actual* loss. Comparing a margin-based critical loss against an actual loss estimated under unrelated demand assumptions is the critical-loss fallacy, and opposing experts will name it. Derive both numbers from one internally consistent demand model (Katz & Shapiro, 2003); (Obrien Wickelgren, 2003).

*Market definition is a tool for analyzing competitive effects, not an end in itself --- a point the general-search market findings in* United States v. Google *(D.D.C. 2024) make concrete.*

## Core tools and workflow

Modern market-definition work cycles through four approaches, each suited to different data environments and legal standards. Each produces artifacts that feed directly into merger simulations ([Chapter 6](chapters/06-mergers.md)) and cartel analysis ([Chapter 5](chapters/05-cartels.md)).

1. **Descriptive analytics.** Price/volume trends, seasonality, cointegration, variance ratios, and shock responses that reveal when products or regions move together. Use `ggplot2` quick looks before heavy modeling.
2. **Demand elasticities & diversion.** Log-log or `fixest` panel regressions, AIDS-lite models, discrete choice on customer-level data (`mlogit`, `apollo`), or churn-based diversion ratios that quantify substitutability. Always document instruments, fixed effects, or controls used.
3. **Critical-loss vs. actual-loss.** Compare hypothetical post-SSNIP losses to observed switching, stating margin assumptions and capacity constraints. Highlight when brand repositioning, supply limits, or multi-product margins break the classic calculations.
4. **Geographic screens.** Shipment flows, travel-time analyses, switching matrices, gravity models, or catchment-area heatmaps built from loyalty data, mobile location pings, or Stats SA transport datasets. Pair with qualitative evidence on delivery commitments or regulatory boundaries.
...
Read `exceeds_critical` directly: `TRUE` means observed switching exceeds the critical loss, so a SSNIP over that segment would lose money and the candidate market is too narrow. `FALSE` means the hypothetical monopolist could profitably raise price, supporting the candidate definition.
...
cl_table <- tibble::tribble(
  ...
  0.25, 0.05, 0.167, 0.08, "Market well-defined (SSNIP profitable; actual < critical)",
  0.25, 0.10, 0.286, 0.12, "Market well-defined (SSNIP profitable; actual < critical)",
  0.40, 0.05, 0.111, 0.15, "Too narrow --- expand the candidate market (actual > critical)"
) |>
  mutate(
...
### Four principles that discipline the exercise

**The Cellophane fallacy.** A SSNIP run from prevailing prices assumes those prices are competitive. In monopolization cases --- Sherman Act §2, Article 102, §8 of the South African Competition Act --- they often are not. A monopolist has already raised price to the point where demand turns elastic, so substitutes look constraining at the margin even though they would not constrain a competitive price. The Supreme Court fell into this trap in the 1956 *du Pont* cellophane decision, accepting flexible wrapping materials as substitutes at du Pont's monopoly price (Us Dupont, 1956). In abuse-of-dominance work, run the test from a competitive benchmark price, or rest the boundary on evidence other than observed substitution at current prices.

**The smallest-market principle.** The hypothetical monopolist test is typically satisfied by many nested candidate markets: if a SSNIP is profitable over a narrow product group, it is usually profitable over any broader group containing it. Convention resolves the ambiguity by selecting the smallest group of products that satisfies the test. State this when defending a narrow market. The question is not whether a broader market would also pass, but whether the narrow one does.

***Brown Shoe* practical indicia.** Courts still ask for the qualitative checklist from *Brown Shoe* (*Brown Shoe v. United States*, 1962): industry or public recognition of the submarket, the product's peculiar characteristics and uses, unique production facilities, distinct customers, distinct prices, sensitivity to price changes, and specialized vendors. Treat the indicia as an evidence-organization device rather than a separate test. Each maps onto something the quantitative tools measure: distinct prices onto price-correlation screens, distinct customers onto switching matrices, sensitivity to price changes onto elasticities.

**Aggregate diversion.** The bridge between diversion ratios and critical loss. The aggregate diversion ratio is the share of sales lost by one product after a price increase that is recaptured by *other products inside the candidate market*. If aggregate diversion exceeds the critical loss, the candidate market passes the breakeven test (Katz & Shapiro, 2003). The same switching matrix that flags unilateral effects in [Chapter 6](chapters/06-mergers.md) therefore closes the market-definition argument here --- estimate it once, use it twice.
...
p_heatmap <- ggplot(route_shares_top, aes(x = carrier, y = route_labeled, fill = share)) +
  ...
  labs(
    ...
    subtitle = "Top 20 routes by volume. HHI > 1,800 = highly concentrated (2023 Guidelines)",
    x = "Carrier",
...
**Interpretation:** Routes with HHI above 1,800 are "highly concentrated" under the 2023 US Merger Guidelines. A merger on such a route triggers the structural presumption only if it also raises HHI by more than 100 points (the presumption alternatively attaches at a combined share of 30% or more with delta-HHI above 100). The visualization shows that many NYC routes are served by only 1-2 carriers with dominant shares, which could support narrow route-level market definitions in merger analysis. In practice, you would supplement this with pricing data, switching patterns, and qualitative evidence about entry barriers.
...
The heatmap above shows that many NYC-origin routes are served by only one or two carriers with dominant shares---exactly the pattern that motivates route-level market definition. In the DOJ's 2013 challenge to the American Airlines/US Airways merger, the government pleaded relevant markets as **city pairs** --- scheduled air passenger service between an origin city and a destination city, pooling nearby airports --- and alleged harm on more than 1,000 of them. Slot-constrained Reagan National (DCA) received separate treatment because the merged carrier would control most of its slots. Even at the city-pair level, the merging carriers' combined shares exceeded the structural presumption thresholds on hundreds of routes; the airport-vs-city-pair choice matters most where a metro area has several substitutable airports, as the NYC heatmap illustrates.
...
route_shares <- tryCatch(
  read.csv("data/derived/airline_route_shares.csv"),
  error = function(e) NULL
)

if (!is.null(route_shares)) {
  ...
  aa_us_routes <- route_shares |>
    ...
    # Structural presumption needs both: post-merger HHI > 1,800 AND delta-HHI > 100
    mutate(post_hhi = hhi + delta_hhi) |>
    filter(post_hhi > 1800, delta_hhi > 100) |>
    arrange(desc(delta_hhi))

  # Top overlap routes — those triggering the structural presumption
  cat("Routes where the AA/US Airways overlap triggers the structural presumption",
      "(post-merger HHI > 1,800 and delta-HHI > 100):\n")
  aa_us_routes |>
    head(10) |>
    mutate(across(c(aa_share, us_share, combined), ~round(., 1)),
           hhi = round(hhi), post_hhi = round(post_hhi),
           delta_hhi = round(delta_hhi)) |>
    knitr::kable(caption = "AA/US Airways Overlap Routes (Top 10 by Delta-HHI)") |>
    print()
} else {
  cat("Route share data not available.",
      "Run program/scripts/05_flights_extended.R to build",
      "data/derived/airline_route_shares.csv.\n")
}
```
...
products <- c("Product_A", "Product_B", "Product_C", "Product_D", "Outside")
shares   <- c(0.30, 0.25, 0.20, 0.15, 0.10)

# Second choices follow market shares among the remaining options (the
# logit-style share-proportional benchmark), except that A and B are close
# substitutes: switchers from A are tilted toward B and vice versa. Without
# that tilt, diversion would mechanically mirror shares and the figure
# could not show excess diversion to the merger partner.
draw_second <- function(first) {
  remaining <- setdiff(products, first)
  w <- shares[match(remaining, products)]
  if (first == "Product_A") w[remaining == "Product_B"] <- 1.5 * w[remaining == "Product_B"]
  if (first == "Product_B") w[remaining == "Product_A"] <- 1.5 * w[remaining == "Product_A"]
  sample(remaining, 1, prob = w / sum(w))
}

# Customer choices: first choice and second choice (if first unavailable)
switching_data <- tibble(
  customer_id = 1:n_customers,
  first_choice = sample(products, n_customers, replace = TRUE, prob = shares)
) |>
  rowwise() |>
  mutate(second_choice = draw_second(first_choice)) |>
  ungroup()
...
- **High diversion between merger parties** (here roughly 45% from A→B, against a share-proportional benchmark near 36%): Strong evidence products compete closely; supports narrow market definition and raises UPP concerns. Compare estimated diversion to the share-proportional benchmark---excess diversion is the signal.
...
**Hospitals & insurers (US).** In FTC v. Advocate Health Care (7th Cir. 2016), the court examined whether a hospital merger would harm commercially insured patients in the Chicago North Shore area. The FTC presented patient origin data showing that 80% of patients traveled less than 15 miles, commercial insurer testimony about limited bargaining leverage with "must-have" hospitals, and survey evidence on patient switching. Defendants countered with broader geographic market definitions based on physician referral patterns and specialty care availability. The court sided with the FTC's narrower market, emphasizing that commercial insurers—not patients—were the direct customers, and their testimony about competitive dynamics was dispositive. **Lesson:** Hospital-insurer competition is a two-stage bargaining process---insurers assemble networks first, patients then choose where to seek care (the Capps-Dranove-Satterthwaite framework). Identify the customer at the stage where the harm operates, here the insurer, and weight that customer's testimony heavily.

**Grocery retail (South Africa).** The 2015-2019 Grocery Retail Market Inquiry combined retailer transaction data, mall lease registers showing exclusive clauses, and micro-level price data by product and area. Investigators used the transaction data to map customer catchment areas, which showed shopping concentrated close to home, and examined whether more distant stores constrained local supermarket pricing. The inquiry documented how long-term exclusive leases between national chains and mall developers foreclosed entry by discounters and smaller rivals in townships and shopping centres, effectively segmenting markets by neighborhood. **Lesson:** Granular transaction data can ground a catchment-area market definition that would otherwise rest on assertion; pair it with qualitative evidence (lease contracts) to show barriers to expansion.

**Tech platforms: Google Search (US/EU).** In US v. Google (2020) and European Commission decisions, market definition turned on whether general search, specialized search (maps, shopping, travel), and social media search were in the same market. Google argued for a broad "online advertising" or "digital information" market. Agencies presented telemetry showing users rarely substitute between general search and social platforms for commercial queries, default contract terms showing Google paid billions to remain the default on Apple's Safari, Mozilla's Firefox, and Android devices (revealing high diversion), and advertiser testimony that search ads have unique intent-based targeting. The narrow "general search" market was upheld. **Lesson:** In zero-price markets, use revealed preference (default payments, multi-homing rates) rather than stated willingness to pay; SSNDQ (Small but Significant Non-transitory Decrease in Quality) can substitute for SSNIP.
...
- **Private Healthcare Market Inquiry (Competition Commission, 2014–2019).** A Commission-appointed panel, chaired by former Chief Justice Sandile Ngcobo, reviewed six years of patient-level claims covering roughly 70% of the 8.8 million beneficiaries in South Africa’s private schemes, combining them with supplier cost data to test alternative geographic definitions for specialist care. The inquiry found Herfindahl indices above 4,000 in several provinces and documented limited patient switching despite tariff differentials, motivating recommendations on supply-side licensing reform and transparency.
- **Grocery Retail Market Inquiry (2015–2019).** Covered in the case box above. Its pairing of transaction-data catchment evidence with mall lease registers is the model for combining quantitative boundaries with the contractual evidence that explains them.
...
Market definition is a tool for finding the constraints that discipline pricing, not an end in itself: draw the boundary where those constraints meaningfully weaken, and say which evidence---margins, switching data, customer testimony, internal documents---placed it there. The quantitative screens are sensitive to their inputs. Critical loss moves with the margin assumption, geographic markets with travel time and freight cost, so report the assumptions next to the conclusions. Keep a record of the candidate markets you screened and rejected; opposing experts will ask.
