...
Mergers ask what happens when two firms combine. Monopolization asks a harder question, and one courts have struggled with for a century: what may a firm that already holds substantial market power do to keep it? Where is the line between competing hard and excluding rivals unlawfully, and how do we measure the harm when conduct crosses it?

Answering them draws on the tools developed so far. Market definition establishes dominance. The IO toolkit supplies pricing and foreclosure models. Research design guides the empirical measurement of competitive effects. Monopolization cases also turn on intent evidence and business justifications, so the quantitative and qualitative strands have to be read together.
...
Section 2 of the Sherman Act in the United States and Article 102 of the Treaty on the Functioning of the European Union (TFEU) frame the legal standards for monopolization and abuse of dominance, but enforcement priorities and evidentiary approaches differ substantially across jurisdictions. The workflow runs through market power assessment, then the specific conduct categories---predation, rebates and loyalty programs, tying and bundling, most-favored-nation clauses, vertical restraints, and refusal to deal---pairing the mechanism in each case with an empirical measurement strategy and the qualitative evidence that supports it. It closes with remedy design and case exemplars from the United States, European Union, and Southern Africa.

For deeper context, consult the case law on exclusionary conduct, including *United States v. Microsoft* (*United States v. Microsoft*, 2001) and the European Commission's decisions on abuse of dominance (EC Article 102 Guidance, 2009). For theoretical foundations, see (Whinston, 2006) and (Salop & Scheffman, 1983) on raising rivals' costs.
...
The European Commission's 2009 *Intel* decision applied a version of this test, asking whether the rebates made it unprofitable for AMD to compete for incremental volumes even when AMD's products were technically competitive. The litigation that followed turned the AEC analysis itself into the battleground. The Court of Justice held in 2017 that the Commission had to engage with Intel's criticisms of its AEC analysis (*Intel*, 2017); on remand the General Court annulled the rebates finding in 2022, identifying errors in the Commission's AEC analysis; and the CJEU dismissed the Commission's appeal in October 2024 (Cjeu Intel, 2024). The Commission's re-adopted €376 million fine (September 2023) covers only the naked restrictions — payments to delay or cancel AMD-based products — not the rebates. The AEC test survives as the analytical framework. The lesson of *Intel* is that the analysis must be executed well enough to withstand an as-efficient cross-examination.
...
# Buying the last 5 units triggers a 10% rebate on ALL 100 units ($1,000).
# The rebate recaptured per incremental unit is $1,000 / 5 = $200, so the
# effective price of those 5 units is 100 - 200 = -$100 each.
list_price <- 100
rebate_value <- 100 * list_price * 0.10        # $1,000 across all 100 units
marginal_units <- 5
rebate_per_unit <- rebate_value / marginal_units  # $200 per incremental unit
effective_price <- list_price - rebate_per_unit   # -$100
...
p_waterfall <- waterfall_steps |>
  ...
  labs(
    ...
    subtitle = "Customer at 95-unit threshold: the last 5 units carry an effective price of -$100",
    x = NULL,
    y = "Price per unit",
    caption = "Scenario: $100 list price, 10% retroactive rebate at 95% volume share.\nThe rebate recaptured per incremental unit ($200) exceeds the list price, so the effective price is -$100.\nA rival with $75 incremental cost cannot profitably compete for these units."
  ) +
  theme_antitrust() +
  theme(
...
**As-efficient competitor (AEC) test:** The waterfall shows that even an equally efficient rival (cost = $75) cannot profitably compete for the marginal units that trigger the rebate: the effective price of those units is -$100. An AEC analysis of this kind sits at the center of EU rebates enforcement, but *Intel* is the cautionary tale on execution rather than a safe harbor. The General Court annulled the Commission's rebates finding in 2022 because its AEC analysis contained errors, and the CJEU dismissed the Commission's appeal in October 2024 (*Intel*, 2017). The test remains analytically central; a flawed AEC analysis is fatal to the case built on it.

### Qualitative evidence for rebates and exclusivity

Internal documents and customer testimony complement the quantitative foreclosure metrics:
...
# Extract coefficients for plotting. fixest names i() terms like
# "rel_period::-3:treated", so pull the (possibly negative) integer after "::".
event_coefs <- broom::tidy(event_fit, conf.int = TRUE) |>
  filter(grepl("rel_period::", term)) |>
  mutate(
    rel_period = as.numeric(sub("^rel_period::(-?[0-9]+).*$", "\\1", term))
  ) |>
...
In the EU *AKZO* case (*AKZO*, 1991), internal documents referencing a plan to eliminate a competitor were central to the finding of abuse, even though the price-cost analysis was contested. The Southern African *Media24* case (discussed later in this chapter) shows the limit of that strategy. The Tribunal's 2015 predation finding rested on prices above avoidable cost but below average total cost, with intent evidence supplying the rest; the Competition Appeal Court overturned it in 2019. Intent could not substitute for a price-cost test the conduct actually failed.
...
*United States v. Microsoft* (*United States v. Microsoft*, 2001) is the canonical example, but read the appellate holding carefully. The D.C. Circuit *vacated* the district court's per se tying ruling and remanded it for rule-of-reason analysis (the government later dropped the claim). What it affirmed was Section 2 monopoly maintenance: the OEM license restrictions, the commingling of Internet Explorer code with Windows, and the Java conduct that together suppressed the middleware threat to Microsoft's operating-system monopoly. For economic analysis of tying and bundling, see (Whinston, 2006). While Microsoft argued integration benefits, the court found that much of the integration could have been accomplished without contractual or technical restrictions on rivals' ability to compete.
...
The enforcement record against Amazon's across-platform price parity was national, not Commission-led: Germany's Bundeskartellamt moved against the clauses in 2013, the UK Office of Fair Trading pursued a parallel investigation, and Amazon dropped the parity requirement across the EU. The European Commission's later Amazon commitments addressed analogous parity clauses in e-books (eu_amazon_parity). The UK record on hotel booking is also more equivocal than often remembered — the CMA closed its online hotel-booking investigation in 2015 without imposing remedies. Where the UK did prohibit wide MFNs was private motor insurance, following the CMA's 2014 market investigation (cma_hotel_booking).
...
(p_dispersion | p_platform_prices) / p_passthrough +
  plot_annotation(
    caption = "Data: Synthetic for illustration. Replace with platform transaction data and hotel pricing across channels.\nSee the Bundeskartellamt's Amazon price-parity case (2013) and the CMA's private motor insurance MFN prohibition (2014) for real applications."
  )
```
...
margin_squeeze_data <- tibble(
  quarter = quarters,
  # Incumbent's wholesale access price to rivals: $45 through Q10,
  # raised by $10 from Q11 onward (the squeeze episode)
  wholesale_price_external = 45 + if_else(quarter >= 11, 10, 0) +
    rnorm(length(quarters), 0, 2),
...
p_margin <- margin_squeeze_data |>
  ...
  geom_vline(xintercept = 10.5, linetype = "dashed", color = "gray50") +
  ...
  annotate("text", x = 10.5, y = 12,
           label = "Wholesale price\n+$10 (Q11)",
           hjust = -0.1, size = 3.2, color = "gray40") +
...
# Panel 2: Price waterfall showing squeeze mechanism (pre-increase quarters)
# Uses the DGP means for Q1-Q10: retail $75, wholesale $45, downstream $22,
# leaving a rival margin of 75 - 45 - 22 = $8. The final bar is a total
# (drawn from zero), not another cumulative step.
rival_margin_avg <- 75 - 45 - 22  # $8
waterfall_data <- tibble(
  ...
  start = c(0, 75, 30, 0),
  end = c(75, 30, rival_margin_avg, rival_margin_avg),
...
p_waterfall_margin <- waterfall_data |>
  ...
  annotate("text", x = 3.5, y = 12,
           ...
           color = "#E69F00", size = 3.5) +
  annotate("text", x = 4, y = rival_margin_avg + 2.5,
           label = paste0("$", rival_margin_avg),
  ...
  scale_y_continuous(labels = scales::dollar_format()) +
  labs(
    title = "Margin waterfall (average Q1-Q10, before the increase)",
    subtitle = "An $8 margin clears the strict AEC test; the Q11 wholesale increase pushes it below zero",
    x = NULL,
...
**Interpretation:** Through quarter 10 the imputed rival margin averages $8 — positive, so no squeeze under the strict as-efficient-competitor test, though already below the roughly $10 needed to fund fixed costs and customer acquisition at scale. From quarter 11 the incumbent raises the wholesale access price by $10 and the margin turns negative: an equally efficient rival — one with the same downstream costs as the incumbent — now loses money on every customer it serves at the incumbent's retail price. That switch from thin-but-positive to negative is what the squeeze test is built to detect. The wholesale comparison adds the discrimination evidence: the incumbent implicitly transfers access to its own retail arm at about $38, below the $45 (and later $55) charged to rivals.
...
- **Media24 community newspapers (Competition Commission v. Media24, Tribunal case 122/CR/Dec12).** Investigators constructed four-year panel data on advertising volumes, pagination, and production costs for the Forum and Gold-Net News titles in Welkom. Forum's ad rates covered avoidable cost but sat below average total cost, so the Tribunal's 2015 predation finding rested on that ATC shortfall plus internal documents targeting rival exit. The Competition Appeal Court overturned the finding in 2019, rejecting the ATC-plus-intent theory as a basis for predation liability. The episode is the region's clearest statement that intent evidence cannot rescue prices that pass the avoidable-cost test.
- **Polypropylene feedstock (Competition Commission v. Sasol Chemical Industries).** The Commission benchmarked Sasol’s propylene and polypropylene prices against export parity and long-run cost, and in 2014 the Tribunal found excessive pricing and imposed a R534 million penalty. The Competition Appeal Court set the finding aside in June 2015. It faulted the cost side of the case — the Tribunal had reallocated Sasol’s actual costs and capital values in ways the CAC found unjustified — and the comparator benchmarks built on them. The reversal is itself the methods lesson: an excessive-pricing finding stands or falls on cost-allocation and comparator choices that survive appellate scrutiny.
...
Interpretation: If the foreclosed share (here 47.1%) exceeds the threshold needed for rivals to reach minimum efficient scale, and contracts are long-duration with staggered renewals, foreclosure concerns are substantial.
{% endhint %}
...
**European Union (Article 102 TFEU):** Applies to firms in a "dominant position." The as-efficient competitor (AEC) test frames pricing abuses post-*Intel* (*Intel*, 2017) — and *Intel* shows the test cuts both ways: the General Court annulled the Commission's rebates finding in 2022 for errors in its AEC analysis, the CJEU dismissed the appeal in October 2024, and the re-imposed €376 million fine covers only the naked restrictions. Explicit recoupment proof is not required for predatory pricing. Self-preferencing is recognized as a standalone theory of harm (*Google Shopping*) (*Google Shopping*, 2017). Remedies tend toward behavioral obligations.
...
## The US record, 2024--2026

For two decades the newest landmark US monopolization judgment was *Microsoft*. Three district court decisions between August 2024 and November 2025 changed that, and each turns on a theme this chapter has developed.

**Google search: liability, then a behavioral remedy.** In August 2024, Judge Mehta found that Google had unlawfully maintained monopolies in general search services and general search text advertising under Section 2 [*United States v. Google LLC*; (Us Google Search Liability, 2024)]. The mechanism was distribution. Exclusive default and preinstallation agreements — including the multibillion-dollar payments that made Google the default on Safari and on Android devices — locked up the channels through which queries arrive and denied rivals the scale needed to compete. The September 2025 remedies opinion (Us Google Search Remedies, 2025) is the document to study alongside step 6 of this chapter's workflow. The court declined the government's request to divest Chrome. It barred exclusive default and preinstallation agreements, required Google to share certain search index and user-interaction data with qualified competitors, and placed compliance under technical-committee oversight. The remedy targets the diagnosed mechanism — foreclosed distribution and denied scale — rather than the firm's structure, the fit logic that [Chapter 8](chapters/08-regulation-remedies.md) develops. Appeals were pending as of this writing, so treat the remedy package as provisional.

**Google ad tech: tying plus monopoly maintenance.** In April 2025, Judge Brinkema found Section 2 liability in two markets — publisher ad servers and ad exchanges — and held that Google unlawfully tied its publisher ad server (DFP) to its exchange (AdX) [*United States v. Google LLC*; (Us Google Adtech, 2025)]. The claim that Google monopolized a third market, advertiser ad networks, failed. The split result rewards close reading: the same defendant and the same integrated stack produced liability only where the evidence established durable power in a properly defined market and a tie that foreclosed rivals. Remedies proceedings followed.

**FTC v. Meta: market power must be proven, and proven current.** In November 2025, Judge Boasberg entered judgment for Meta, holding that the FTC had failed to prove that Meta currently holds monopoly power in the alleged market for personal social networking services [*FTC v. Meta Platforms, Inc.*; (Us Ftc Meta, 2025)]. The case failed at this chapter's step 2. Monopoly power must be shown now, in a market that survives substitution evidence — and evidence of competition from short-video rivals undercut the boundaries the complaint had drawn years earlier. Shares computed within a contested market definition could not carry the burden.

Read together, the three cases confirm the framework rather than replace it. Conduct theories — default tying, technical tying — win where both the foreclosure mechanism and current market power are proven; they fail where market power is not. And the search remedy marks where US relief currently sits: behavioral obligations plus data access, with structural separation held in reserve.
...
A vertically integrated incumbent sells a wholesale input to downstream rivals at $45 and sells at retail for $75. An equally efficient downstream rival incurs $22 of its own downstream costs.

a. Apply the margin-squeeze (equally-efficient-competitor) test: what margin is left for the downstream rival?
b. Is there a squeeze on these numbers?
c. Below what retail price would an equally efficient rival be squeezed (driven to a negative margin)?

{% hint style="success" %}
**Worked answer**

a. Imputed margin = retail - wholesale - downstream cost = 75 - 45 - 22 = **$8**.
b. The margin is positive ($8), so an equally efficient rival can survive -- **no squeeze** on these numbers.
c. The rival breaks even at retail = 45 + 22 = **$67**; below $67 the spread turns negative and an equally efficient rival is squeezed.
{% endhint %}

## Looking ahead

Monopolization analysis produces artifacts—foreclosure calculations, effective price computations, margin squeeze tests—that reappear in the chapters ahead. **[Chapter 8](chapters/08-regulation-remedies.md)** addresses what happens after a liability finding: how to design remedies that cure the diagnosed harm and how to evaluate whether they work. Many monopolization cases end in consent decrees with behavioral obligations, and the next chapter explains how to structure and monitor those commitments.

The exclusionary conduct frameworks here also carry into **[Chapter 9](chapters/09-digital-markets.md)**, where platform self-preferencing, default tying, and data leverage cases build on the same foundations.
