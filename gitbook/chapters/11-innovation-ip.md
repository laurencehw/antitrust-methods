...
- **EC Qualcomm decisions (2018-2019) and their fate on review:** The Commission fined Qualcomm €997 million for exclusivity payments to Apple (2018) and separately for predatory pricing aimed at excluding Icera (2019). The General Court annulled the Apple-exclusivity decision in 2022 (Eu Qualcomm Gc, 2022), citing procedural failings and an effects analysis that did not match the decision's theory of harm; it largely upheld the Icera fine in 2024, with a modest reduction. The pair shows the Commission's willingness to pursue SEP-adjacent conduct---and the EU courts policing how it does so.

{% hint style="info" %}
**Case box: FTC v. Qualcomm (2019--2020)**

The FTC alleged that Qualcomm used its monopoly in CDMA and premium LTE modem chips to extract supracompetitive royalties through a "no license, no chips" policy: it refused to supply chips unless manufacturers accepted its SEP licensing terms (*FTC v. Qualcomm*, 2020). Judge Koh's sweeping district court ruling found antitrust violations across licensing and chip markets, ordering Qualcomm to renegotiate agreements at FRAND rates. The Ninth Circuit reversed, holding that Qualcomm's licensing practices---even if they violated FRAND commitments---did not constitute anticompetitive conduct *in chip markets* because OEMs had no alternative chipset suppliers regardless of licensing terms. The decision effectively requires plaintiffs to show harm to competition (not just to competitors) in a properly defined market, raising the bar for SEP-based antitrust claims in the United States. The EU's parallel Qualcomm cases looked, for a time, like the receptive contrast: in 2018 the Commission fined Qualcomm €997 million for paying Apple to source baseband chips exclusively from it. The General Court annulled that decision in 2022 (Case T-235/18), finding procedural irregularities in how the Commission built its file and an as-efficient-competitor analysis that did not match the theory of harm the decision actually charged. The separate Icera predatory-pricing fine fared better, largely surviving review in 2024 with a small reduction. The transatlantic lesson is narrower than "the EU is friendlier": both systems demand that the economic analysis presented match the theory of harm pleaded. The FTC lost on market definition and harm; the Commission lost on process and fit.
{% endhint %}

These cases qualify the usual story of jurisdictional divergence. EU authorities pursue SEP-adjacent conduct more readily than US agencies, but the General Court's Qualcomm annulment shows the EU courts demanding the same discipline---an effects analysis that matches the theory charged---that the Ninth Circuit demanded of the FTC.
...
ggplot(stack, aes(xmin = start, xmax = end, ymin = 0, ymax = 1, fill = component)) +
  ...
  # antitrust_colors is a named vector keyed to other levels; unname so the
  # palette is recycled over these components instead of falling back to gray
...
```
Replace with real pay-for-delay cases (Actavis-style) or EU decisions (Servier, Lundbeck). Use FDA Orange Book, EMA data, or South African SAHPRA registration records for entry timelines.
...
The following charts are built for PatentsView data---the USPTO's public bulk download (the bulk files must be fetched manually; the API route is deprecated). When the files are absent, the chunk substitutes a clearly labelled synthetic series with the same schema so the book renders from a fresh clone. Patent counts are an imperfect proxy for innovation, but they are the most widely available and standardized measure, and they feature in merger review (as indicators of innovation incentives) and in SEP/FRAND disputes (where patent portfolios determine licensing leverage).
...
# Real source: PatentsView bulk downloads (USPTO), aggregated to
# data/raw/patents_total_by_year.csv (year, total_utility_patents) and
# data/raw/patents_by_sector_year.csv (year, sector, patent_count).
# Guarded read with a schema-matched synthetic fallback.
patents_raw <- tryCatch({
  list(
    total  = read.csv("data/raw/patents_total_by_year.csv",
                      stringsAsFactors = FALSE),
    sector = read.csv("data/raw/patents_by_sector_year.csv",
                      stringsAsFactors = FALSE)
  )
}, error = function(e) NULL)

patents_synth <- is.null(patents_raw) ||
  ("data_source" %in% names(patents_raw$total) &&
     any(patents_raw$total$data_source == "synthetic_fallback"))

if (is.null(patents_raw)) {
  # Synthetic fallback for illustration only: exponential growth with noise,
  # calibrated to the broad shape of US utility patent grants.
  set.seed(1976)
  yrs_total <- 1976:2024
  total_df <- data.frame(
    year = yrs_total,
    total_utility_patents = round(
      70000 * exp(0.035 * (yrs_total - 1976)) *
        exp(rnorm(length(yrs_total), 0, 0.03))
    )
  )
  synth_sectors <- c("Computer/Electronics", "Pharmaceuticals",
                     "Semiconductors", "Clean Energy", "Motor Vehicles")
  synth_base    <- c(40000, 12000, 15000, 3000, 8000)
  synth_growth  <- c(0.040, 0.010, 0.030, 0.070, 0.015)
  sector_df <- do.call(rbind, lapply(seq_along(synth_sectors), function(i) {
    yrs <- 2000:2024
    data.frame(
      year = yrs,
      sector = synth_sectors[i],
      patent_count = round(
        synth_base[i] * exp(synth_growth[i] * (yrs - 2000)) *
          exp(rnorm(length(yrs), 0, 0.04))
      )
    )
  }))
} else {
  total_df <- patents_raw$total
  sector_df <- patents_raw$sector
}

patents_src <- if (patents_synth) {
  "Synthetic data for illustration; replace with PatentsView bulk download (USPTO)."
} else {
  "Source: PatentsView (USPTO bulk download). Real data."
}
...
p1 / p2 +
  plot_annotation(caption = patents_src)
```

![US utility patent grants, total and by technology sector. The in-figure caption states whether the rendered series is the real PatentsView data or a labelled synthetic stand-in.](../images/innovation-patent-trends-1.png)
...
3. **Case discussion.** Compare the outcomes in *FTC v. Qualcomm* (US: district-court liability reversed by the Ninth Circuit) and the EC Qualcomm matters (the €997M Apple-exclusivity fine annulled by the General Court in 2022; the Icera predatory-pricing fine largely upheld in 2024). What do the three outcomes, taken together, say about the evidentiary discipline reviewing courts impose on SEP-adjacent antitrust theories in each jurisdiction?
