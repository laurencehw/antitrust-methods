...
The approach prioritizes clean identification: shocks---institutional details, policy boundaries, timing quirks---that separate treated and control groups. It shows the raw data first, with event-study plots before regression tables. And it demands falsification: placebo tests on pre-treatment periods or unaffected markets, where any "effect" would expose a broken design.
...
```
   ...
   control group?            SIMULATION                   overcharge or
          |                  (IO toolkit and              lost profits
    +-----+-----+            mergers chapters)            (litigation chapter)
    |           |
    v           v
   YES          NO
    |           |
...
3.  **"Causal Inference for the Brave and True"** (Alves, 2022): Covers intermediate topics including machine-learning approaches to inference. [matheusfacure.github.io/python-causality-handbook](https://matheusfacure.github.io/python-causality-handbook/)
...
Three debates recur in nearly every case. First, **stated vs. revealed preferences**: surveys can illuminate multi-sided platforms where transaction data are limited (app stores, adtech), but agencies often prefer revealed behavior. One compromise is to calibrate survey responses against observable churn or clickstream metrics, as seen in the DOJ’s ad-tech inquiries (*United States v. Google (Search)*, 2023). Second, **parallel trends under innovation**: in markets with rapid product cycles (semiconductors, fintech), clean control groups may not exist. Explain how you use feature-adjusted indices, synthetic controls, or machine-learning prognostics to approximate counterfactuals, and disclose residual risk. Third, **placebo tests**: courts increasingly expect falsification exercises. In wage-fixing litigation (e.g., physical-therapist staffing (*United States v. Jindal*, 2021), US tech no-poach cases (Us High Tech Nopoach, 2015)), teams ran placebo periods and alternative employee groups to show effects were specific to collusive periods rather than macro shocks---labor market concentration analysis is developed in [Chapter 10](chapters/10-labor-markets.md).
{% endhint %}

## Visualizations

### Event study around a merger announcement
This figure structures an event-study diagnostic around the February 2013 American Airlines/US Airways announcement---for the *rivals*, not the merging parties. Neither party can appear in the window: American's parent AMR was in Chapter 11 and traded over the counter as AAMRQ, US Airways traded as LCC, and neither delisted history is served by free equity data sources (AAL only began trading in December 2013, after the merger closed). That gap is itself a methods lesson. Survivorship bias in free price data silently removes exactly the firms a merger retrospective cares about, so check ticker histories before designing the study. Rival returns still carry signal: positive rival abnormal returns on announcement are consistent with expected market power (softer competition ahead), negative ones with expected efficiencies. Replace tickers and the `event_date` to align with a specific matter.
...
2. **Data/code.** The chapter's event study shows rivals only, because the merging parties' tickers (AAMRQ, LCC) are delisted and free sources no longer serve them. (a) Modify the scaffold to analyze rival cumulative abnormal returns around a different merger announcement of your choosing, and interpret the sign: what would positive rival returns suggest about expected competitive effects? (b) Obtain AAMRQ and LCC price histories from a survivorship-bias-free source (e.g., CRSP via WRDS) and add the merging parties to the figure. What does comparing party and rival returns add to the rivals-only picture?
