...
Much of the rest of the book runs through this chapter. The demand models, cost estimates, and bargaining frameworks here feed the merger simulations in @sec-mergers, the foreclosure analysis in @sec-monopolization, and the platform work in @sec-digital-markets.
...
::: {.callout-important title="Key IO Formulas"}
...
Units note: the UPP and GUPPI rows state the index form, a share of price. In currency units, UPP~A~ = D~A→B~ × (P~B~ − C~B~) − E~A~, the form used in @sec-mergers; dividing by P~A~ gives GUPPI~A~ = D~A→B~ × M~B~ × P~B~/P~A~. With equal prices the index reduces to Diversion × Margin, as in the table.
:::
...
::: {.callout-note title="Jurisdictional comparison: IO tools in practice"}
...
The example below uses a cross-section of breakfast cereal brands, the setting of @nevo_2001, to illustrate the mechanics. With 10 inside goods and two predictors, we have adequate degrees of freedom.
...
::: {.callout-note title="Case box: UPP analysis in AT&T/T-Mobile (2011)"}
...
::: {.callout-tip title="Running example: Airline demand and UPP"}
...
# Pre-pulled FRED series (program/scripts/01_fred_extended.R):
# WTI crude oil (DCOILWTICO, daily) and US retail gasoline (GASREGW, weekly).
read_fred <- function(path) {
  tryCatch(
    read_csv(path, show_col_types = FALSE) |>
      transmute(date = as.Date(date), value),
    error = function(e) NULL
  )
}

crude_raw <- read_fred("data/raw/fred_crude_oil.csv")
gas_raw <- read_fred("data/raw/fred_gas_prices.csv")

pt_synth <- is.null(crude_raw) || is.null(gas_raw)
if (pt_synth) {
  # Labelled synthetic fallback so the chapter renders offline; calibrated
  # with ~0.5 contemporaneous pass-through so the figure tells the same story.
  set.seed(2024)
  months <- seq(as.Date("2015-01-01"), as.Date("2025-12-01"), by = "month")
  n <- length(months)
  log_crude <- log(60) + cumsum(rnorm(n, 0, 0.06))
  log_gas <- log(2.5) + 0.5 * (log_crude - log(60)) + cumsum(rnorm(n, 0, 0.01))
  crude_raw <- tibble(date = months, value = exp(log_crude))
  gas_raw <- tibble(date = months, value = exp(log_gas))
}

# Aggregate both series to monthly averages
...
pass_df <- to_monthly(crude_raw) |> rename(crude = value) |>
  inner_join(to_monthly(gas_raw) |> rename(gas = value), by = "month") |>
  arrange(month) |>
  filter(month >= as.Date("2015-01-01")) |>
  mutate(
    crude_index = crude / first(crude) * 100,
    gas_index = gas / first(gas) * 100,
    # Log-differences for the pass-through regression (levels are spurious)
    dlog_crude = log(crude) - log(lag(crude)),
    dlog_gas = log(gas) - log(lag(gas)),
    dlog_crude_l1 = lag(dlog_crude),
    dlog_crude_l2 = lag(dlog_crude, 2)
  ) |>
  drop_na()

# Time series plot (levels, for context only -- not the regression)
pass_long <- pass_df |>
  select(month, `Crude oil (cost)` = crude_index, `Retail gasoline` = gas_index) |>
  pivot_longer(-month, names_to = "series", values_to = "index")

p1 <- ggplot(pass_long, aes(x = month, y = index, color = series)) +
  geom_line(linewidth = 1.1) +
  scale_color_manual(values = c("Crude oil (cost)" = "#D55E00",
                                "Retail gasoline" = "#0072B2")) +
  labs(
    title = "Crude Oil Cost vs. Retail Gasoline Price",
    subtitle = "Levels shown for context; the pass-through regression uses log-differences",
    x = NULL,
    y = "Index (start = 100)",
    color = NULL
  ) +
...
# Pass-through regression in log-differences with two monthly lags
pt_model <- lm(dlog_gas ~ dlog_crude + dlog_crude_l1 + dlog_crude_l2,
               data = pass_df)
pt_now <- coef(pt_model)[["dlog_crude"]]
pt_cum <- sum(coef(pt_model)[c("dlog_crude", "dlog_crude_l1", "dlog_crude_l2")])
pt_r2 <- summary(pt_model)$r.squared

p2 <- ggplot(pass_df, aes(x = dlog_crude, y = dlog_gas)) +
  geom_point(alpha = 0.5, color = "#0072B2") +
  geom_smooth(method = "lm", se = TRUE, color = "#D55E00", linewidth = 1.2) +
  annotate("text", x = min(pass_df$dlog_crude), y = max(pass_df$dlog_gas),
           label = paste0("Contemporaneous: ", round(pt_now, 2),
                          "\nCumulative (3 mo.): ", round(pt_cum, 2),
                          "\nR² = ", round(pt_r2, 2)),
           hjust = 0, vjust = 1, size = 4, fontface = "bold") +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Pass-Through Regression (Log-Differences)",
    subtitle = "Monthly % change in retail gasoline vs. % change in crude",
    x = "Crude oil, monthly log change",
    y = "Retail gasoline, monthly log change"
  ) +
  theme_antitrust() +
  theme(plot.title.position = "plot")

# Combined plot, with the caption stating the data source honestly
src_caption <- if (pt_synth) {
  "Synthetic data for illustration only. Run program/scripts/01_fred_extended.R to fetch the real FRED series."
} else {
  "Data: FRED series DCOILWTICO (WTI crude) and GASREGW (US retail gasoline), monthly averages."
}
p1 / p2 + plot_annotation(caption = src_caption)

# Summary statistics
pass_summary <- tibble::tibble(
  metric = c("Contemporaneous coefficient", "Cumulative coefficient (3 months)",
             "R-squared (differences)", "Observations"),
  value = c(round(pt_now, 3), round(pt_cum, 3), round(pt_r2, 3), nrow(pass_df))
)

cat("\nPass-through analysis summary:\n")
knitr::kable(pass_summary, digits = 3,
             caption = "Pass-through estimated in monthly log-differences")
```
...
## Looking ahead
The demand estimates, diversion matrices, and pass-through diagnostics introduced here form the quantitative backbone of the chapters ahead. In **@sec-cartels**, we apply variance screens and structural break tests to detect collusion, while in **@sec-mergers** we feed these IO primitives directly into merger simulations. The same bargaining and pass-through tools reappear in **@sec-monopolization** and **@sec-digital-markets**, where multi-sided platform dynamics add further complexity.
