...
Instead of asking "what controls do I need to fix my model?", design-based thinking asks "where does the variation come from?" If you cannot draw a Directed Acyclic Graph (DAG) showing how the treatment was assigned independent of the outcome, no amount of regression control will save the analysis [@pearl_2009]. This "reduced form" approach contrasts with the "structural" methods used in merger simulation (see @sec-io-toolkit), which rely on theoretical models to estimate "deep parameters".
...
::: {.callout-note title="Method selection guide"}
```
...
|:-------|:------------|:---------------|:---------------------|
| **DiD** | Policy/merger with clear timing, parallel control group | Parallel trends | Merger retrospectives (@sec-mergers), cartel breakdowns (@sec-cartels) |
...
::: {.callout-tip title="Getting up to speed"}
...
3.  **"Causal Inference for the Brave and True"** [@alves_2022]: Covers intermediate topics including machine-learning approaches to inference. [matheusfacure.github.io/python-causality-handbook](https://matheusfacure.github.io/python-causality-handbook/)
...
**Diff-in-diff and event studies**: Always test for pre-trend equivalence using graphical diagnostics and formal tests. Dynamic specifications (leads/lags) are persuasive in telecom or energy cases where policy shocks phase in, and in remedy evaluations as we show in @sec-regulation-remedies. The DOJ used this approach in Spirit/JetBlue analyses [@us_spirit_jetblue_2023], while South African regulators applied similar diagnostics to evaluate grocery supplier rebates.  
...
**Surveys and diversion studies**: Define the decision-maker (consumer vs. procurement lead), sampling frame, and recall period before crafting questions. The FTC’s Staples/Essendant review [@ftc_staples_essendant_2019] and the Competition Commission South Africa’s ride-hailing inquiry both relied on carefully screened respondents to estimate diversion ratios---a key input for market definition as discussed in @sec-market-definition. Pre-test instruments to detect anchoring or order effects, and document weighting schemes.  
...
```{r}
#| label: research-design-merger-event
#| message: false
#| warning: false
#| echo: false
#| cache: true
#| cache.extra: !expr file.mtime("data/raw/finance_airlines.csv")
#| fig-width: 9
#| fig-height: 5
library(dplyr)
library(ggplot2)
...
rivals <- c("DAL", "LUV", "UAL")    # rival carriers; SPY proxies the market
...
if (!is.null(prices)) {
  prices <- prices |>
    filter(symbol %in% c(rivals, "SPY"),
           date >= event_date - 180, date <= event_date + 180) |>
    group_by(symbol) |>
    arrange(date) |>
  ...
  cars <- prices |>
    ...
    filter(rel_day >= -30, rel_day <= 30, !is.na(abnormal_ret)) |>
    group_by(symbol) |>
    arrange(rel_day) |>
  ...
  ggplot(cars, aes(x = rel_day, y = car, color = symbol)) +
    ...
    labs(
      title = "Rival Carriers' Cumulative Abnormal Returns: AA/US Airways Announcement",
      subtitle = "Delta (DAL), Southwest (LUV), and United (UAL) vs. SPY benchmark (±30 trading days).\nMerging parties absent by construction: AAMRQ and LCC delisted; see text.",
  ...
  cat("Airline price data not available.",
      "Run program/scripts/02_finance_extended.R to build",
      "data/raw/finance_airlines.csv.\n")
}
```
