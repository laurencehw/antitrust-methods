...
ggplot(event_data, aes(x = day, y = car, color = firm)) +
  ...
  theme_antitrust()
```
Replace synthetic returns with `tidyquant::tq_get()` using actual tickers and event dates from FRAND rulings (e.g., Optis v. Apple [@uk_optis_apple_2023], Unwired Planet [@uk_unwired_planet_2017]).
...
The Supreme Court rejected the "scope of the patent" test that had shielded reverse payment settlements from antitrust scrutiny, holding instead that such agreements must be evaluated under the rule of reason [@us_actavis_2013]. The Court reasoned that a large unexplained payment from a brand-name manufacturer to a generic challenger suggests the patent holder has doubts about patent validity---and that paying to delay entry is an exercise of market power, not a legitimate exploitation of patent rights. The decision established that the *size of the payment* relative to litigation costs and saved royalties is the key indicator of anticompetitive harm. Post-Actavis, pharmaceutical companies restructured settlements to avoid explicit cash transfers, using instead "authorized generics," supply agreements, and co-promotion deals that obscure the economic equivalent of reverse payments. Enforcers must now trace the value of these non-cash transfers to demonstrate that settlements effectively compensate generics for staying out of the market.
:::
...
#| fig-cap: "Reverse-payment (pay-for-delay) price effects. Synthetic data for illustration only."
#| message: false
#| warning: false
library(fixest)
library(dplyr)
...
```{r}
#| label: innovation-patent-trends
#| message: false
#| warning: false
#| echo: true
#| fig-width: 10
#| fig-height: 7
#| fig-cap: "US utility patent grants, total and by technology sector. The in-figure caption states whether the rendered series is the real PatentsView data or a labelled synthetic stand-in."
library(dplyr)
library(ggplot2)
library(patchwork)
source("program/R/helpers.R")
...
#| fig-cap: "R&D response to a policy change. Synthetic data for illustration only."
#| message: false
#| warning: false
library(fixest)
library(dplyr)
...
Innovation and IP issues are increasingly central to merger review and monopolization enforcement. The survival models, event studies, and patent network tools in this chapter carry forward into **@sec-litigation-practice**, which covers how to present technical expert testimony alongside economic evidence. The killer acquisitions framework connects to **@sec-mergers** for transactions involving nascent competitors, and the SEP/FRAND analysis informs remedy design in **@sec-regulation-remedies**. When real data become available (PatentsView bulk downloads, FDA Orange Book, SAHPRA filings), replace the synthetic examples with actual patent networks, entry timelines, and R&D panels.
