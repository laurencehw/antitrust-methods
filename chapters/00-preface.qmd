...
**For classroom use:** Work straight through. Start with the orientation to antitrust institutions (@sec-orientation) and research design (@sec-research-design), then the analytical core---market definition (@sec-market-definition), the IO toolkit (@sec-io-toolkit), and the practice areas (cartels, mergers, monopolization). The later chapters on digital markets, labor, and innovation apply those methods to harder, newer terrain.

**For practitioners:** Use it as a reference. Each chapter opens with its goals and a roadmap, so the relevant section is easy to find. The empirical appendix (@sec-empirical-appendix) collects reusable code templates and diagnostics.
