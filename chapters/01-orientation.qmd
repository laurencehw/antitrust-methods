...
milestones <- tibble::tribble(
  ...
  1982, "DOJ Merger Guidelines", "Guidance",
  1998, "US v. Microsoft", "Case",
  2013, "US v. Apple eBooks", "Case",
  2019, "Competition Amendment\nAct (SA)", "Statute",
  2020, "EC Google Shopping", "Case",
  2023, "Joint Merger Guidelines", "Guidance"
) |>
  dplyr::mutate(
    y = dplyr::if_else(dplyr::row_number() %% 2 == 0, 1.2, -1.2),
...
ggplot(milestones, aes(x = year, y = y, color = type)) +
  ...
  geom_text(aes(label = event, vjust = vjust),
            size = 3.5, show.legend = FALSE, lineheight = 0.9) +
  scale_y_continuous(limits = c(-2, 2)) +
  scale_x_continuous(labels = scales::comma, breaks = seq(1880, 2030, 20)) +
  scale_color_manual(values = c("Statute" = "#D55E00", 
                                 "Guidance" = "#0072B2", 
                                 "Case" = "#009E73")) +
  labs(
    title = "Key Antitrust Milestones",
    subtitle = "Blend statutory, guidance, and litigation anchors to frame workflows",
...
ggplot(hhi, aes(date, value)) +
  ...
  labs(title = "US Bank Concentration (HHI)", 
       x = NULL, y = "HHI",
       caption = "Source: FRED series HHMSDODNS") +
