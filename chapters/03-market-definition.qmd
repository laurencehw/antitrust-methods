...
This chapter uses the causal toolkit from @sec-research-design to draw those boundaries from data and qualitative evidence---boundaries that hold up when a regulator or a court pushes on them.
...
::: {.callout-note title="SSNIP Test Decision Flowchart"}
```
...
A low critical loss does not, by itself, justify a narrow market. High pre-merger margins are themselves evidence of inelastic demand: the same margins that lower the critical loss also predict a low *actual* loss. Comparing a margin-based critical loss against an actual loss estimated under unrelated demand assumptions is the critical-loss fallacy, and opposing experts will name it. Derive both numbers from one internally consistent demand model [@katz_shapiro_2003; @obrien_wickelgren_2003].
...
Modern market-definition work cycles through four approaches, each suited to different data environments and legal standards. Each produces artifacts that feed directly into merger simulations (@sec-mergers) and cartel analysis (@sec-cartels).
...
The workflow mirrors @sec-research-design's research design steps: scoping memo → data inventory → qualitative plan → estimation → memo/slide deck. Revisit `chapters/13-empirical-appendix.qmd` for templates.
...
observed <- tibble::tribble(
  ...
  mutate(
    ...
    exceeds_critical = observed_share_loss > critical_loss
  )
...
::: {.callout-tip title="Running example: Airline market definition"}
...
**Why it matters:** High diversion between products suggests they compete closely. In merger analysis, high diversion between merging parties signals unilateral effects concerns (more in @sec-mergers). For market definition, diversion patterns reveal which products belong in the same market.

This example shows how to compute diversion from customer-level switching or choice data.

```{r}
#| label: diversion-ratios
#| message: false
#| warning: false
library(dplyr)
library(ggplot2)
...
::: {.callout-note title="Case box: Market definition in practice"}
...
::: {.callout-note title="Debate: Is market definition still necessary?"}
...
Market definition answers "which products compete?" The **Industrial Organization Toolkit** in **@sec-io-toolkit** answers the next question: how much pricing power does each firm have *within* those boundaries? It measures that power with concentration indices (HHI), pricing relationships, and entry barriers.
...
2. **Geographic definitions**: Confirm that shipment-flow maps and customer testimony align with the geographic markets you'll assume in @sec-io-toolkit.
...
@sec-io-toolkit takes market definitions as fixed and looks inside those markets: how firms set prices, whether entry threatens incumbents, and what evidence points to market power. The switching matrices from this chapter become diversion ratios for the merger simulations in @sec-mergers.
