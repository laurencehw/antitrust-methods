...
The workflow above identifies *what* to do; this section shows *how* to do it. Each diagnostic below produces artifacts that feed directly into merger simulations (@sec-mergers) and cartel analysis (@sec-cartels). Use these specific diagnostics to anchor your analysis:
...
With market boundaries now defined, we turn in **@sec-io-toolkit** to the **Industrial Organization Toolkit**: quantifying market power *within* those boundaries using concentration measures (HHI), pricing relationships, and entry barriers. Market definition answers "which products compete?"; IO analysis answers "how much pricing power does each firm have?"
...
In @sec-io-toolkit, we'll assume market definitions are fixed and focus on what happens *inside* those markets: how firms set prices, whether entry threatens incumbents, and what evidence suggests market power. You'll use the switching matrices from this chapter to compute diversion ratios for merger simulations in @sec-mergers.
