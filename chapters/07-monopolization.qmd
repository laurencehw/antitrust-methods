...
For deeper context, consult the case law on exclusionary conduct, including *United States v. Microsoft* [@us_microsoft_2001] and the European Commission's decisions on abuse of dominance [@ec_article102_guidance_2009]. For theoretical foundations, see @whinston_2006 and @salop_scheffman_1983 on raising rivals' costs.

## Market power and the monopolization workflow

::: {.callout-note title="Monopolization Analysis Workflow"}
```
...
```{r}
#| label: monopoly-effective-price-waterfall
#| message: false
#| warning: false
#| fig-width: 10
#| fig-height: 8
...
```{r}
#| label: monopoly-contract-rollout-event
#| message: false
#| warning: false
#| fig-width: 10
#| fig-height: 6
...
In the EU *AKZO* case [@eu_akzo_1991], internal documents referencing a plan to eliminate a competitor were central to the finding of abuse, even though the price-cost analysis was contested. The Southern African *Media24* case (discussed later in this chapter) shows the limit of that strategy. The Tribunal's 2015 predation finding rested on prices above avoidable cost but below average total cost, with intent evidence supplying the rest; the Competition Appeal Court overturned it in 2019. Intent could not substitute for a price-cost test the conduct actually failed.
...
```{r}
#| label: monopoly-tying-bundling-impacts
#| message: false
#| warning: false
#| fig-width: 10
#| fig-height: 7
...
**Looking ahead:** For more on defaults and choice screens in digital markets, see @sec-digital-markets. For damages estimation in tying cases, see @sec-litigation-practice.
...
```{r}
#| label: monopoly-mfn-price-dispersion
#| message: false
#| warning: false
#| fig-width: 10
#| fig-height: 7
...
**Looking ahead:** For more on platform competition and two-sided markets, see @sec-digital-markets. For MFN-related damages estimation, see @sec-litigation-practice.
...
For FRAND-committed standard-essential patents (SEPs), the analysis shifts to whether licensing terms comply with the FRAND obligation and whether injunctions or threats thereof constitute hold-up. See @sec-innovation-ip for detailed treatment of SEPs.
...
::: {.callout-note title="Method box: Foreclosure share calculation"}
...
```{r}
#| label: monopoly-foreclosure-shares
#| message: false
#| warning: false
library(dplyr)
library(ggplot2)
...
::: {.callout-note title="Method box: Event study around contract rollout"}
...
::: {.callout-note title="Method box: price-cost test with suction effect"}
```{r}
...
::: {.callout-note title="Jurisdictional comparison: Monopolization standards"}
...
**European Union (Article 102 TFEU):** Applies to firms in a "dominant position." The as-efficient competitor (AEC) test frames pricing abuses post-*Intel* [@eu_intel_2017] — and *Intel* shows the test cuts both ways: the General Court annulled the Commission's rebates finding in 2022 for errors in its AEC analysis, the CJEU dismissed the appeal in October 2024, and the re-imposed €376 million fine covers only the naked restrictions. Explicit recoupment proof is not required for predatory pricing. Self-preferencing is recognized as a standalone theory of harm (*Google Shopping*) [@eu_google_shopping_2017]. Remedies tend toward behavioral obligations.
