...
This chapter provides a structured workflow for analyzing monopolization claims. We begin with market power assessment, drawing on tools from @sec-market-definition and @sec-io-toolkit. We then turn to specific conduct categories: predation, rebates and loyalty programs, tying and bundling, most-favored-nation clauses, vertical restraints, and refusal to deal. For each, we pair theoretical mechanisms with empirical measurement strategies and qualitative evidence. The chapter closes with remedy design and a set of case exemplars showing how different jurisdictions approach the same exclusionary practices.

Code boxes compute effective prices, assess foreclosure shares, and visualize event patterns around contract adoptions. Qualitative evidence boxes point to the documentary and testimonial sources that complement quantitative analyses. Case boxes draw on landmark decisions from the United States, European Union, and Southern Africa.
